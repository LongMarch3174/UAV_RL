...
class DoubleDQN():
	def __init__(self, n_actions, n_features=8, n_hidden=64, learning_rate=0.005, reward_decay=0.9, e_greedy=0.9,
				replace_target_iter=200, memory_size=3000, batch_size=32, e_greedy_increment=None, double_q=False):
	...
	'''def learn(self):
		...
		# we use q_eval4next to get which action was chosed by eval network in s_(t+1)
	...
	def plot_cost(self):
		...
		for i in range(len(self.cost_his)):
			math.append(self.cost_his[i].detach().numpy())

		with open('./excel/dqn_loss.csv', 'w', newline='') as file:
			writer = csv.writer(file)
			writer.writerow(["1", "2"])
			for i in range(len(self.cost_his)):
				writer.writerow([i,math[i]])
		plt.plot(np.arange(len(math)), math)
		plt.ylabel('Cost')
		plt.xlabel('training steps')
		plt.savefig("lose.jpg")
		print("213")
		plt.show()'''
